...
class RewardApi extends BaseApi {
  ...
  Future<BaseResponse<RewardInfo>> getCurrentReward() {
    return get("/rewards/current/state").then((response) => processResponse(response, parseReward)).catchError((error) {
      print(error);
    });
  }

  Future<BaseResponse<int>> joinReward(int id, List<UserMatch> couples) {
    final jsonArray = couples.map((c) => {"toUser": c.id, "match": c.idMatch}).toList();
